...
class Ajb extends CI_Controller
{
    ...
    function getDescRumahSelled()
    {
        ...
        $rows = $this->ajb_m->getDescRumahSelled($idRumah);
        if ($rows) {
            foreach ($rows as $row)
    ...
    function getAngsInfo1()
    {
        ...
        $rows = $this->ajb_m->getAngsInfo1($idPenj);
        if ($rows) {
            foreach ($rows as $row)
            $array = array(
                'baris' => 1,
    ...
    function getAngsInfo2()
    {
        ...
        $rows = $this->ajb_m->getAngsInfo2($idPenj,$tglTrans);
        if ($rows) {
            foreach ($rows as $row)
                $array = array(
                    'baris' => 1,
    ...
    function home()
    {
        $menuId = $this->home_m->get_menu_id('ajb/home');
        ...
        if (isset($_POST["btnSimpan"])) {
            ...
            $this->template->load('template/template3', 'transaksi/ajb_v', $data);
        }
    }
    ...
    function simpan()
    {
        ...
        $model_trans = $this->ajb_m->ubahMasterJual($data_trans,$idPenj);

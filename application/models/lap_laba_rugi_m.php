...
class Lap_laba_rugi_m extends CI_Model {
	...
	function get_total_pendapatan($tgl_trans,$user_id){
		$sql = "select sum(saldo_akhir) as total_pendapatan from web_temp_perkiraan where LEFT(web_temp_perkiraan.kode_perk,1)=4 AND user_id = '".$user_id."' 
		AND tanggal <= '".$tgl_trans."'";
		$query = $this->db->query($sql)->row()->total_pendapatan;
		return $query;
	}
	function get_total_biaya($tgl_trans,$user_id){
		$sql = "select sum(saldo_akhir) as total_biaya from web_temp_perkiraan where LEFT(web_temp_perkiraan.kode_perk,1)=4 AND user_id = '".$user_id."' 
		AND tanggal <= '".$tgl_trans."'";

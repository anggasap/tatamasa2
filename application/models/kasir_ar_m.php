...
class Kasir_ar_m extends CI_Model {
	public function getBookingAll()
	{
		$sql="SELECT tj.master_id,mj.id_rumah,mj.id_cust,mr.nama_rumah,mc.nama_cust,tj.jml_trans
			  ...
			  where tj.kode_trans = '100' and tj.status_akuntansi = 1 and tj.status_kasir=0";
...
//		$this->db->where ( 'T.STATUS_AKTIF <>', 3 );
	...
	function getDataCetakSt($idadv)
	{
		$sql = "select ma.id_settle_adv,ma.id_kyw,mk.nama_kyw,mk.dept_kyw,md.nama_dept,ma.tgl_trans,
				(ma.jml_uang_paid + ma.jml_uang_oupaid) as jml_uang,ma.keterangan,tb.kode_perk,tb.nama_kdbayar,
				tb.norek_bank,tdp.id_proyek,tdp.voucher_no,tdp.tgl_trans as tgl_voucher,cf.kode_cflow,
				(select a.nama_cflow from master_cashflow a where a.kode_cflow = cf.kode_cflow) as nama_cf 
				from master_settle_adv ma
				inner join master_karyawan mk on ma.id_kyw = mk.id_kyw
				inner join master_dept md on md.id_dept = mk.dept_kyw
				inner join cpa cf on ma.id_settle_adv = cf.id_master
				inner join trans_detail_perk tdp on tdp.master_id = ma.id_settle_adv
				inner join type_bayar tb on tb.kode_perk = tdp.kode_perk where tdp.trans_id = '".$idadv."'
				group by ma.id_settle_adv";
		$query = $this->db->query($sql);
		return $query->result(); // returning rows, not row
	}
	...
	function updateBooking_statuskasir($data_model2,$id_master){
		$this->db->trans_begin();

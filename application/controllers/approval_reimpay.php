...
class Approval_reimpay extends CI_Controller
{
	function __construct()
	{
		parent::__construct();
		$this->load->model('home_m');
		$this->load->model('approval_reimpay_m');
	...
	function home(){
		$menuId = $this->home_m->get_menu_id('approval_reimpay/home');
		...
		$this->template->load ( 'template/template3', 'approval/approval_reimpay_v',$data );
	}	
    function approval(){
        $id	 				= trim($this->input->post('idReimpay'));
		...
		if($hak == 'Head Department'){
			...
			$model = $this->approval_reimpay_m->updateReimpay($data,$id);
	...
	public function getAllRb(){
		...
		$rows = $this->approval_reimpay_m->getRbAll();
		$data['data'] = array();
		foreach( $rows as $row ){
			...
			$user = $this->approval_reimpay_m->getPayTo(trim($row->pay_to));
			$array = array(
					'idRb' 			=> trim($row->id_reimpay),
	...
	function getDescReimpay(){
		...
		$rows = $this->approval_reimpay_m->getDescRb($idRb);
		if($rows){
			foreach ( $rows as $row )

...
class Approval_settlement extends CI_Controller
{
	function __construct()
	{
		parent::__construct();
		$this->load->model('home_m');
		$this->load->model('approval_settle_m');
	...
	function home(){
		$menuId = $this->home_m->get_menu_id('approval_settlement/home');
		...
		$this->template->load ( 'template/template3', 'approval/approval_settle_v',$data );
	}	
    function approval(){
        $id	 				= trim($this->input->post('idSettle'));
		...
		if($hak == 'Head Department'){
			...
			$model = $this->approval_settle_m->update($data,$id);
		}elseif($hak == 'Keuangan'){
			$data = array(
        ...
        if($model){
            $array = array(
                'act'	=> $id.' '.$gmStatus.' '.$tanggal.' '.$gmKeterangan,
                'tipePesan'=>'success',
	...
	public function getAllSettle(){
		$this->CI =& get_instance();
		$rows = $this->approval_settle_m->getSettleAll();
		$data['data'] = array();
		foreach( $rows as $row ){
			...
			$user = $this->approval_settle_m->getPayTo(trim($row->pay_to));
			$array = array(
					'idSettle' 		=> trim($row->id_settle_adv),
					'namaKyw' 		=> trim($row->nama_kyw),
					'deptKyw' 		=> trim($g),
					'jumlah' 		=> trim(number_format($row->jml_uang_paid, 2, ',', '.')),
					'nama_user'		=> $user,
	...
	function getDescSettle(){
		$this->CI =& get_instance();
		$idRb = $this->input->post ( 'idRb', TRUE );
		$rows = $this->approval_settle_m->getDescSt($idRb);
		if($rows){
			foreach ( $rows as $row )
				...
				$jml = number_format($row->jml_uang_oupaid,2);
				$jml1 = str_replace('-','',$row->jml_uang_oupaid);
				if($row->jml_uang_oupaid <= 0){
					$jml1 = str_replace('-','',$row->jml_uang_oupaid);
					$jml_uang_oupaid = '('.$jml1.')';
				}else{
				...
				$array = array(
						...
						'jml_uang' 				=> $jml_uang,
						'jml_uang_paid'			=> $jml_uang_paid,
						'jml_uang_oupaid'		=> $jml_uang_oupaid,
						'tgl_jt' 				=> $tgl_jt,
						'pay_to' 				=> $user,

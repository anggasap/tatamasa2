...
class Laporan_ar extends CI_Controller
{
	...
	function getCustAll(){
		$this->CI =& get_instance();//and a.kcab_id<>'1100'
        $rows = $this->laporan_ar_m->getCusAll();
        $data['data'] = array();
        foreach($rows as $row){
            $array = array(
                'idCust' => trim($row->id_cust),
                'nama' => trim($row->nama_cust),
	...
	function cetak()
    {
    	if($this->auth->is_logged_in() == false){
			...
			$data['customer'] = $this->laporan_ar_m->getCustomerById($customer);
			$data['list'] = $this->laporan_ar_m->getAllArAdv($rumah,$customer);
